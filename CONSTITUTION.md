...
5. **Behind → ahead conversion, where convertible.** Push is
   necessary but not sufficient. Even perfect push is post-facto:
   a completion event can only fire after the failure. There are two
   error horizons — a KNOWN bad pattern is caught *ahead* by a
   pre-merge gate (it never lands); a NOVEL pattern is caught
   *fast-behind* by this loop. The loop MUST classify each novel
   failure for **gate-convertibility**:
     - A gate can only assert a **repo property** checkable at PR
       time. A deterministic class (a forbidden code pattern, a
       missing manifest entry, a malformed config) IS convertible:
       the ACTOR — a developer or a capable agent — writes the new
       static rule (Amendment 8). Enforcement code is authored by the
       trusted, reviewed actor — never auto-generated by an untrusted
       scribe. The loop has no scribe: the structured failure record
       is handed to the actor directly.
     - An infra / transient / flake failure (cert-provisioning race,
       cloud-API timeout, DB transient, consistency race) is NOT a
       repo property and is NOT gate-convertible. Its artifact is
       retry logic or a runbook note. Forcing a gate produces the
       wrong artifact and causes gate sprawl — itself a §0 smell.
   A recurrence is a §0 process violation ONLY for a convertible
   class whose conversion was skipped. A recurring flake, or a
   genuinely new sub-case, is not a process violation.
